...
func _process(delta):
	...
	var ActiveCamera = get_node("../Camera") #Use the path to your camera
	...
	ZoomRatio = clamp(ZoomRatio, 2, 4.5)
	
	#ZoomRatio is a scalar so we need to turn it into a vector
	ActiveCamera.zoom = ActiveCamera.zoom.linear_interpolate( (Vector2(ZoomRatio,ZoomRatio)), 0.15)
